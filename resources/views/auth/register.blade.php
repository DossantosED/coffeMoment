@extends('layouts.main', ['class' => 'off-canvas-sidebar', 'activePage' => 'register', 'title' => __('Coffee Moment')])
    ...
    <div class="col-lg-4 col-md-6 col-sm-8 ml-auto mr-auto">
      <form class="form" method="POST" action="{{ route('register') }}">
        ...
        <div class="card card-login card-hidden mb-3">
          ...
          <div class="card-body ">
            <p class="card-description text-center">{{ __('Crear una cuenta') }}</p>
            <div class="bmd-form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
              <div class="input-group">
                ...
                <input type="text" name="name" class="form-control" placeholder="{{ __('Usuario') }}" value="{{ old('name') }}" required autocomplete="name" autofocus>
              </div>
              @if ($errors->has('name'))
            ...
            <div class="form-check mr-auto ml-3 mt-3">
              <label class="form-check-label">
                ...
                {{ __('Estoy de acerdo con las ') }} <a href="#">{{ __('Normas') }}</a>
              </label>
            </div>
          </div>
